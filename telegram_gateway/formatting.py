...
as ``**heading**`` and complete ATX heading prefixes such as ``### heading`` so
model formatting hints do not appear as literal punctuation. It does not
interpret links, code, single stars or unmatched delimiters.
...
def _is_fence_opener(text: str, opening: int, run_end: int, marker: str) -> bool:
    if run_end - opening < 3 or not _has_fence_indent(text, opening):
        return False
    line_end = text.find("\n", run_end)
    if line_end < 0:
        line_end = len(text)
    return marker != "`" or "`" not in text[run_end:line_end]


def _matching_fence_run(
    text: str,
    marker: str,
    minimum: int,
    start: int,
) -> tuple[int, int] | None:
    """Find the active fence's closing run, ignoring foreign fence markers."""

    position = text.find(marker, start)
    while position >= 0:
        run_end = position + 1
        while run_end < len(text) and text[run_end] == marker:
            run_end += 1
        line_end = text.find("\n", run_end)
        if line_end < 0:
            line_end = len(text)
        ...
        if (
            ...
            return position, run_end
        position = text.find(marker, run_end)
    return None


def _matched_code_mask(text: str) -> list[bool]:
    """Mark inline code and fences in one left-to-right, non-nesting pass.

    Once a backtick or tilde fence opens, only a valid run of that same marker can
    close it. Foreign fence-looking lines are code, not new openers that may leak
    protection beyond the active fence.
    """

    protected = [False] * len(text)
    cursor = 0
    while cursor < len(text):
        next_backtick = text.find("`", cursor)
        next_tilde = text.find("~", cursor)
        candidates = [position for position in (next_backtick, next_tilde) if position >= 0]
        opening = min(candidates) if candidates else -1
        if opening < 0:
            break
        marker = text[opening]
        run_end = opening + 1
        ...
        is_fence = _is_fence_opener(text, opening, run_end, marker)
        closing = (
            _matching_fence_run(text, marker, len(delimiter), run_end)
            if is_fence
            else (
                _matching_inline_backtick_run(text, delimiter, run_end) if marker == "`" else None
            )
        )
        if closing is None:
            if is_fence:
...
def _unwrap_atx_heading_prefixes(text: str) -> str:
    """Remove complete prose-level ATX prefixes while keeping literals intact.

    One to six hashes followed by horizontal whitespace is the unambiguous ATX
    prefix generated by the adviser. Hashes in code, URLs and identifiers stay
    literal; so do escaped, seven-hash and no-space forms. Up to three leading
    spaces are part of ATX syntax, while four-space indented code is left alone.
    """
    ...
    parts: list[str] = []
    cursor = 0
    for match in _ATX_HEADING_PREFIX.finditer(rendered):
        marker_start, marker_end = match.span("marker")
        if any(protected[marker_start:marker_end]):
            continue
        line_end = rendered.find("\n", match.end())
        if line_end < 0:
            line_end = len(rendered)
        content = rendered[match.end() : line_end].strip(" \t\r")
        if len(match.group("marker")) == 1 and content.isdecimal():
            # A copied numeric reference (notably ``# 4501789``) is ambiguous
            # and more useful intact than guessed to be an H1 heading.
            continue
        parts.append(rendered[cursor : match.start()])
        cursor = match.end()
    if not parts:
        return rendered
    parts.append(rendered[cursor:])
...
def _trim_transport_edges(text: Any) -> str:
    """Drop blank transport padding without stripping content indentation."""
...
def _split_oversized(block: str, limit: int) -> list[str]:
    ...
    while len(remaining) > limit:
        ...
        if cut >= 0:
            if cut:
                out.append(remaining[:cut].rstrip("\r"))
...
def split_message(text: str, *, limit: int = SAFE_CHUNK_CHARS) -> list[str]:
    ...
    """
    text = _trim_transport_edges(text)
    if not text:
        return []
    ...
    for block in text.split("\n\n"):
        block = _trim_transport_edges(block)
        if not block:
            continue
    ...
    if current:
        chunks.append(current)
    return [c for c in (_trim_transport_edges(c) for c in chunks) if c]

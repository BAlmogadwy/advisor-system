"""Drop spent link tokens and old update receipts.
...
class Command(BaseCommand):
    help = "Delete spent Telegram link tokens and old update receipts. Dry-run unless --apply."
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if not options["apply"]:
            ...
            tokens = TelegramLinkToken.objects.filter(created_at__lt=cutoff).count()
            receipts = TelegramUpdateReceipt.objects.filter(received_at__lt=cutoff).count()
            self.stdout.write(

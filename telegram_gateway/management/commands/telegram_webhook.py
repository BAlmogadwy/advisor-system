...
class Command(BaseCommand):
    ...
    def _set(self, token: str, *, keep_pending: bool) -> None:
        ...
        if urlparse(base).scheme != "https":
            # Telegram refuses plain HTTP, and a link built on http:// would send
            # the student's session over the wire in clear.
            raise CommandError(f"TELEGRAM_PUBLIC_BASE_URL must be https:// (got {base!r}).")
        if not secret:
        ...
        if not result.get("ok"):
            raise CommandError(f"setWebhook refused: {result.get('description', '(no reason)')}")

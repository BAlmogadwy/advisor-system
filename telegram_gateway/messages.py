...
HELP_LINKED = (
    ...
    "ستجد في الرسالة التالية أمثلة لأسئلة يمكنني مساعدتك فيها.\n\n"
    "الأوامر:\n"
...
#: Sent as its own message after linking and again after linked `/help`. Keeping
#: one reviewed text for both paths prevents the onboarding examples and the
#: help examples from drifting apart as adviser capabilities evolve.
STARTER_QUESTIONS = (
    ...
    "٩. أنشئ لي عدة خيارات لجدول مقترح من دون تعارضات، مع الإبقاء على الشُعب "
    "الموجودة في جدولي كما هي.\n"
    "١٠. ما المدة التقديرية المتبقية لإكمال خطتي بدءًا من مقررات الفصل التي يوصي "
    "بها النظام؟\n"
    "١١. بناءً على جدولي المسجّل فعليًا، ما المدة التقديرية المتبقية لإكمال خطتي؟\n"
    "١٢. إذا حذفت [رمز المقرر] من جدولي المسجّل فعليًا، فهل تتغير الخطة التقديرية "
    "أو عدد الفصول المتبقية؟"
)
...
PRIVACY = (
    ...
    "• ما يُعالَج ويُرسل عبر تيليجرام: نص سؤالك وإجابة المرشد، ومعرّف حسابك في تيليجرام، "
    "لغرض واحد هو الرد عليك تحت هويتك الجامعية المُتحقَّق منها. وعندما يعرض المرشد جدولك "
    "الحالي أو المتوقع، أو يبني جدولًا مقترحًا، فقد يرسل صورة للجدول. وقد تتضمن الصورة الفصل "
    "الدراسي المقصود، والمقررات والشُّعب، وأيام المحاضرات وأوقاتها، والعبء الدراسي بالساعات، "
    "وقيود التخطيط التي طلبتها مثل مقرر إلزامي أو شعبة مثبّتة، والمقررات التي لم تُدرج وأسباب "
    "تعذّر تحقيق بعض القيود. وعندما يعرض المرشد محاكاة إكمال الخطة، فقد يرسل صورة لخريطة "
    "المسار تتضمن الفصل المرجعي للتخطيط، والمقررات المجتازة والمفترض اجتيازها والمتوقعة، "
    "وعلاقات المتطلبات، والمتطلبات التي لم تحسمها المحاكاة، وتغييرات السيناريو التي طلبتها.\n"
    "• يحتفظ تيليجرام بالرسائل والصور المُرسلة وفق سياساته، ويمكن تنزيل صورة الجدول أو إعادة "
    "توجيهها مثل أي وسائط أخرى في تيليجرام.\n"
    ...
    "حسابك الجامعي وفق سياسة الاحتفاظ المعمول بها في المنصة، ولا يحذفه إلغاء الربط. كما أن "
    "إلغاء الربط لا يحذف الرسائل أو صور الجداول التي سبق إرسالها إلى تيليجرام.\n\n"
    "Telegram is an external cloud service and this chat is not end-to-end encrypted. "
    "Your question, the advisor's answer, and your Telegram account identifier are processed "
    "to reply under your verified university identity. When the advisor displays your current "
    "or expected timetable, or builds a proposed one, it may send an image of that timetable. "
    "It may also send an image of a simulated graduation-plan map. These images may include the "
    "planning baseline; completed, baseline, projected, or unresolved courses; prerequisite links; "
    "scenario changes and unresolved requirements. A timetable image may include courses and "
    "sections; class days and times; credit "
    "load; requested planning constraints such as must-take courses or pinned sections; and "
    "unplaced courses or reasons a constraint could not be satisfied. Telegram retains sent "
    "messages and images under its policies, and timetable images can be downloaded or forwarded. "
    "/unlink revokes the connection immediately, but it does not delete your university advisor "
    "history or messages and timetable images already sent to Telegram."
)
